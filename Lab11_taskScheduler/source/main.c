...
int main(void) {
    ...
    task2.TickFct = &pauseLED0ToggleSMTick;//Function pointer for the tick
    //Task 3 (toggleLED1SM)
    ...
    task3.TickFct = &pauseLED1ToggleSMTick;//Function pointer for the tick
    //Task 4 (displaySM)
    ...
    unsigned long int GCD = task[0]-period;
    TimerSet(GCD);
    TimerOn();
    ...
    while (1) {/*
		...
		for(i = 0; i < numTasks; i++){//Scheduler code
			if(tasks[i]->state = tasks[i]->period){//Task is ready to tick
